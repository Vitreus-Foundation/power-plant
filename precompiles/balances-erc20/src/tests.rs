...
fn transfer_not_enough_funds() {
    ExtBuilder::default()
        // to make the total issuence big enough, we define the second account
...
fn deposit(data: Vec<u8>) {
    ExtBuilder::default()
        .with_balances(vec![(CryptoAlith.into(), 1000), (Precompile1.into(), 1000)])
        .build()
        .execute_with(|| {
            ...
            // Check precompile balances are still the same.
