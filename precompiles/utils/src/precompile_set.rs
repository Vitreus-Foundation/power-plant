...
fn common_checks<R: pallet_evm::Config, C: PrecompileChecks>(
    ...
    if !callable_by_precompile {
        if let IsPrecompileResult::Answer { is_precompile: true, .. } =
            <R as pallet_evm::Config>::PrecompilesValue::get().is_precompile(caller, 0)
        {
    ...
    C: PrecompileChecks,
{
    ...
    #[inline(always)]
    fn is_precompile(&self, address: H160) -> bool {
        let is_precompile = match self.precompile_set.is_precompile(address, 0) {
...
impl<R: pallet_evm::Config, P: PrecompileSetFragment> PrecompileSet for PrecompileSetBuilder<R, P> {
    ...
    fn is_precompile(&self, address: H160, _gas: u64) -> IsPrecompileResult {
        IsPrecompileResult::Answer {
            is_precompile: self.inner.is_precompile(address),
            extra_cost: 0,
        }
    }
}
