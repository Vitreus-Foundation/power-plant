...
use {
    ...
    evm::gasometer::{GasCost, Gasometer},
    fp_evm::{Log, PrecompileHandle},
    frame_support::sp_runtime::SaturatedConversion,
};
...
impl<T: PrecompileHandle> PrecompileHandleExt for T {
    ...
    fn record_log_costs_manual(&mut self, topics: usize, data_len: usize) -> EvmResult {
        ...
        let log_cost = GasCost::Log { n: topics.saturated_into(), len: data_len.into() };
        self.record_cost(gasometer.gas_cost(log_cost, 0)?)?;
