...
use vitreus_power_plant_runtime::UncheckedExtrinsic;
...
impl InitPolkadotBlockBuilder for Client {
    ...
    fn init_polkadot_block_builder_at(
        ...
        // let last_timestamp = self.runtime_api().get_last_timestamp(hash).expect("Get last timestamp");
        ...
        let minimum_period = BasicExternalities::new_empty()
            .execute_with(|| vitreus_power_plant_runtime::MinimumPeriod::get());
        ...
        let slot_duration = BasicExternalities::new_empty()
            .execute_with(|| vitreus_power_plant_runtime::SLOT_DURATION);
