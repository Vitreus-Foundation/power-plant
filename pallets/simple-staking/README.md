...
> Below information is from the original substrate's frame `pallet-staking`,
> which our `simple-staking` is based on. Some things won't apply to the
> `simple-staking` pallet. Basically, `simple-staking` is `pallet-staking`, but 
> without nominators and voting.
