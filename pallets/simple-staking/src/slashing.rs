...
use crate::{
    BalanceOf, Config, Error, Exposure, NegativeImbalanceOf, OffendingValidators, Pallet, Perbill,
...
pub(crate) struct SlashParams<'a, T: 'a + Config> {
    ...
    /// The exposure of the stash.
    pub(crate) exposure: &'a Exposure<BalanceOf<T>>,
...
pub(crate) fn compute_slash<T: Config>(
    ...
    let own_slash = params.slash * params.exposure.0;
    if params.slash * params.exposure.0 == Zero::zero() {
