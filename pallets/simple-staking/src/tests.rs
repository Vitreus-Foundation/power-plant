...
use frame_support::{
    ...
    dispatch::{GetDispatchInfo, WithPostDispatchInfo},
    pallet_prelude::*,
...
fn basic_setup_works() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_eq!(Staking::eras_stakers(active_era(), 11), Exposure(1000),);
        assert_eq!(Staking::eras_stakers(active_era(), 21), Exposure(1000),);

        // initial total stake = 1000 (11) + 1000 (21) + 500 (31)
...
fn rewards_should_work() {
    ExtBuilder::default().session_per_era(3).build_and_execute(|| {
        ...
        assert_eq_error_rate!(
            ...
            init_balance_10 + total_payout_0 * 2 / 3, // 100 is 2/3 of 150
            2,
        );
        assert_eq_error_rate!(Balances::total_balance(&11), init_balance_11, 2);
        assert_eq_error_rate!(
            Balances::total_balance(&20),
            init_balance_20 + total_payout_0 / 3, // 50 is 2/3 of 150
            2,
        );
        ...
        assert_eq_error_rate!(
            Balances::total_balance(&10),
            init_balance_10 + (total_payout_0 * 2 / 3 + total_payout_1),
            2,
        );
        assert_eq_error_rate!(Balances::total_balance(&11), init_balance_11, 2);
        assert_eq_error_rate!(
            Balances::total_balance(&20),
            init_balance_20 + total_payout_0 / 3,
            2,
        );
        assert_eq_error_rate!(Balances::total_balance(&21), init_balance_21, 2);
    });
...
fn no_candidate_emergency_condition() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            Validators::<Test>::insert(11, prefs);
...
fn nominating_and_rewards_should_work() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        // re-validate with 41 (41 is idle in the mock)
        assert_ok!(Staking::validate(RuntimeOrigin::signed(40), Default::default()));

        // Set payee to controller.
        assert_ok!(Staking::set_payee(RuntimeOrigin::signed(10), RewardDestination::Controller));
        assert_ok!(Staking::set_payee(RuntimeOrigin::signed(20), RewardDestination::Controller));
        assert_ok!(Staking::set_payee(RuntimeOrigin::signed(30), RewardDestination::Controller));
        assert_ok!(Staking::set_payee(RuntimeOrigin::signed(40), RewardDestination::Controller));

        // give the man some money
        let initial_balance = 1000;
        for i in [10, 11, 20, 21].iter() {
        ...
        assert_eq_error_rate!(Balances::total_balance(&10), initial_balance + payout_for_10, 2);
        assert_eq_error_rate!(Balances::total_balance(&20), initial_balance_20 + payout_for_20, 2,);
    });
}
...
fn cannot_transfer_staked_balance() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_eq!(Staking::eras_stakers(active_era(), 11).0, 1000);
...
fn validator_payment_prefs_work() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        let reward_of_10 = shared_cut + taken_cut;
...
fn bond_extra_and_withdraw_unbonded_works() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_ne!(Staking::eras_stakers(active_era(), 11), Exposure(1100));
        ...
        assert_eq!(Staking::eras_stakers(active_era(), 11), Exposure(1100));
...
fn reward_to_stake_works() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert_eq!(Staking::eras_stakers(active_era(), 11), Exposure(1000));
            assert_eq!(Staking::eras_stakers(active_era(), 21), Exposure(2000));
            ...
            ErasStakers::<Test>::insert(0, 21, Exposure(69));
            <Ledger<Test>>::insert(
                20,
                StakingLedger {
                    stash: 21,
            ...
            assert_eq!(Staking::eras_stakers(active_era(), 11), Exposure(1000));
            assert_eq!(Staking::eras_stakers(active_era(), 21), Exposure(69));
            ...
            assert_eq!(
                ...
                Exposure(1000 + total_payout_0 / 2)
            );
            assert_eq!(Staking::eras_stakers(active_era(), 21), Exposure(69 + total_payout_0 / 2));
        });
}
...
#[test]
fn switching_roles() {
    // Test that it should be possible to switch between roles (validator, idle) with minimal
    // overhead.
    ExtBuilder::default().build_and_execute(|| {
        // Reset reward destination
        for i in [10, 20, 30] {
            assert_ok!(Staking::set_payee(RuntimeOrigin::signed(i), RewardDestination::Controller));
        }
...
fn bond_with_little_staked_value_bounded() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert_eq!(Staking::eras_stakers(active_era(), 2), Exposure(0));
...
fn reward_validator_slashing_validator_does_not_overflow() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        let exposure = Exposure::<Balance>(stake);
        ...
        ErasStakers::<Test>::insert(0, 11, Exposure(stake + 1));
        ...
        assert_eq!(Balances::total_balance(&11), 1);
    })
}
...
fn slashing_performed_according_exposure() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        on_offence_now(
            &[OffenceDetails { offender: (11, Exposure(500)), reporters: vec![] }],
...
fn reporters_receive_their_slice() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_eq!(Staking::eras_stakers(active_era(), 11), Exposure(initial_balance));
...
fn subsequent_reports_in_same_span_pay_out_less() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_eq!(Staking::eras_stakers(active_era(), 11).0, initial_balance);
...
fn invulnerables_are_not_slashed() {
    ...
    ExtBuilder::default().invulnerables(vec![11]).build_and_execute(|| {
        ...
        let _exposure = Staking::eras_stakers(active_era(), 21);
...
fn dont_slash_if_fraction_is_zero() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        // we need to exced the 75% threshold to force new era
        on_offence_now(
            &[OffenceDetails {
                offender: (21, Staking::eras_stakers(active_era(), 21)),
                reporters: vec![],
...
fn remove_multi_deferred() {
    ExtBuilder::default().slash_defer_duration(2).build_and_execute(|| {
        ...
        on_offence_now(
            &[OffenceDetails { offender: (69, exposure), reporters: vec![] }],
            &[Perbill::from_percent(25)],
        );
...
fn slash_kicks_validators_not_nominators_and_disables_nominator_for_kicked_validator() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        on_offence_now(
            &[OffenceDetails { offender: (11, exposure_11), reporters: vec![] }],
            &[Perbill::from_percent(10)],
        );
        ...
        mock::start_active_era(2);
        let exposure_11 = Staking::eras_stakers(active_era(), 11).0;
        let exposure_21 = Staking::eras_stakers(active_era(), 21).0;

        // 11's expo is reduced.
        assert_eq!(exposure_11, 900);
        assert_eq!(exposure_21, 1000);
    });
}
...
fn non_slashable_offence_doesnt_disable_validator() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        on_offence_now(
            &[OffenceDetails { offender: (21, exposure_21), reporters: vec![] }],
            &[Perbill::from_percent(25)],
        );
...
fn offence_threshold_triggers_new_era() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            on_offence_now(
                &[OffenceDetails { offender: (31, exposure_31), reporters: vec![] }],
                &[Perbill::zero()],
            );
...
fn claim_reward_at_the_last_era_and_no_double_claim_and_invalid_claim() {
    // should check that:
    // * rewards get paid until history_depth
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_eq!(
            Balances::total_balance(&10),
            init_balance_10 + total_payout_1 + total_payout_2,
        );
    });
}
...
fn six_session_delay() {
    ExtBuilder::default().initialize_first_session(false).build_and_execute(|| {
        ...
        assert_eq!(<Staking as SessionManager<_>>::new_session(init_session + 6), Some(val_set));
...
fn payout_stakers_handles_weight_refund() {
    ...
    ExtBuilder::default().has_stakers(false).build_and_execute(|| {
        ...
        let _info = call.get_dispatch_info();
...
fn payout_to_any_account_works() {
    ExtBuilder::default().has_stakers(false).build_and_execute(|| {
        ...
        assert_ok!(Staking::set_payee(RuntimeOrigin::signed(10), RewardDestination::Account(42)));
...
fn chill_other_works() {
    ExtBuilder::default()
        ...
        .min_validator_bond(1_500)
        .build_and_execute(|| {
            let _initial_validators = Validators::<Test>::count();
            for i in 0..15 {
                let a = 2 * i;
                let b = 2 * i + 1;
...
fn cannot_set_unsupported_validator_count() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        // We don't have election/max winners, so it doesn't produce an error like the original test
        // does
        // the original test
        ...
        // );
        // ours
        assert_ok!(Staking::set_validator_count(RuntimeOrigin::root(), 51));
    })
}
