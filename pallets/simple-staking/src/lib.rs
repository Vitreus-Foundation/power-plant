...
//!
//! ```
//! use pallet_simple_staking::{self as staking};
//!
...
#[derive(
    PartialEq, Eq, PartialOrd, Ord, Clone, Default, Encode, Decode, RuntimeDebug, TypeInfo,
)]
pub struct Exposure<Balance: HasCompact>(#[codec(compact)] pub Balance);
...
impl<AccountId, Balance: HasCompact + Zero> UnappliedSlash<AccountId, Balance> {
    ...
    pub fn default_from(validator: AccountId) -> Self {
        Self { validator, own: Zero::zero(), reporters: vec![], payout: Zero::zero() }
    }
}
...
impl<T: Config> Convert<T::AccountId, Option<Exposure<BalanceOf<T>>>> for ExposureOf<T> {
    fn convert(validator: T::AccountId) -> Option<Exposure<BalanceOf<T>>> {
        <Pallet<T>>::active_era()
