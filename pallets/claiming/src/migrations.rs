...
pub mod v1 {
    ...
    pub fn migrate_to_double_map<T: Config>() -> Weight {
        ...
        if StorageVersion::get::<Pallet<T>>() == 0 {
            for (key, value) in v0::Claims::<T>::iter() {
                reads += 1;
                writes += 1;
            ...
            let _ = v0::Claims::<T>::clear(u32::MAX, None);
        }
    ...
    pub struct VersionMigrateV0ToV1<T>(PhantomData<T>);

    impl<T: Config> UncheckedOnRuntimeUpgrade for VersionMigrateV0ToV1<T> {
    ...
    /// A type alias that defines the complete migration process for transitioning Claims storage from version 0 to version 1.
    pub type MigrateV0ToV1<T> = VersionedMigration<
        0,
        1,
        VersionMigrateV0ToV1<T>,
        Pallet<T>,
...
pub mod v0 {
    ...
    pub(crate) type Claims<T: Config> =
        StorageMap<Pallet<T>, Identity, EthereumAddress, BalanceOf<T>>;
}
