...
//! # Module Overview
//!
//! This Rust module implements the slashing logic for validators and cooperators in a Substrate-based
//! blockchain. Slashing is a key mechanism used to enforce good behavior among validators, ensuring
//! network security and integrity by penalizing misbehaving nodes. The module includes functions to
//! determine, apply, and manage slashing spans and to handle the disabling of validators found to be
//! violating protocol rules.
//!
//! # Key Features and Functions
//!
//! - **Validator Slashing**:
//!   - `slash_stake(params: SlashParams<T>)`: The main function to apply slashing to a validator's
//!     stake. It takes general slashing parameters and determines how much of the validator's stake
//!     should be slashed, updating the associated data structures accordingly. Slashing is applied
//!     proportionally across active and unlocking funds to ensure a fair and accurate penalty.
//!
//! - **Kicking Out Offending Validators**:
//!   - `kick_out_if_recent(params: SlashParams<T>)`: This function is used to remove a validator from
//!     the active set if the misbehavior occurred during the most recent slashing span. The function
//!     checks if the offence matches the current era span and ends the span if necessary.
//!   - `add_offending_validator(params: &SlashParams<T>)`: Adds an offending validator to the
//!     `DisabledValidators` list and disables them from the network if required by the current
//!     `DisablingStrategy`. This function ensures that validators who misbehave are appropriately
//!     marked and removed from the active set if their actions warrant it.
//!
//! - **Cooperator Slashing**:
//!   - `slash_cooperators(params: SlashParams<T>, validator_slash: Perbill) -> Balance`: This function
//!     slashes the cooperators of a misbehaving validator. It accepts general parameters and the prior
//!     slash percentage applied to the validator. The function ensures that cooperators who bonded
//!     their tokens with a slashed validator also face a proportional penalty, reinforcing the idea
//!     that cooperators must carefully choose validators to bond with.
//!
//! - **Slashing Span Management**:
//!   - `fetch_spans<T>(...) -> SlashingSpans`: Retrieves the slashing spans associated with a validator,
//!     including the history of previous slashes and the current span. This is critical for applying
//!     correct penalties, as it determines the amount and timing of the slash for validators who may
//!     have misbehaved repeatedly.
//!   - `end_span()`: Ends the current slashing span, creating a new span to record future offences.
//!     This ensures that slashing data is segmented correctly over time, enabling accurate tracking
//!     of a validator's slashing history and preventing recurrent offenders from evading penalties.
//!
//! # Access Control and Security
//!
//! - **Controlled Slashing and Disabling**: The slashing functions are designed to be invoked by
//!   privileged entities within the runtime, such as the staking or governance pallets. This ensures
//!   that only authorized actions can lead to the slashing of funds or the disabling of validators.
//! - **Consistency in Disabled Validators**: The `add_offending_validator` function includes logic to
//!   keep the `DisabledValidators` list sorted and to verify that disabled validators are consistently
//!   handled throughout the network. This helps maintain data integrity and prevents inconsistencies
//!   when managing the list of offenders.
//!
//! # Developer Notes
//!
//! - **Disabling Strategy**: The module supports a configurable `DisablingStrategy` that determines
//!   whether and how validators should be disabled based on their offences. This strategy provides
//!   flexibility in how strictly the network handles misbehavior and can be adjusted according to
//!   governance decisions.
//! - **Span Management and Tracking**: The use of `SlashingSpans` helps in managing and tracking
//!   slashing events over time. This is particularly useful for implementing increasing penalties for
//!   repeated misbehavior, as it maintains a record of all previous slashes associated with a validator.
//! - **Validation of Recent Misbehavior**: The `kick_out_if_recent` function includes a check to verify
//!   if the misbehavior is from the most recent span before kicking out a validator. This is an important
//!   safeguard to ensure that validators are not penalized arbitrarily, but rather in response to recent
//!   and verified misbehavior.
//!
//! # Usage Scenarios
//!
//! - **Proportional Slashing**: The `slash_stake` and `slash_cooperators` functions are used to ensure
//!   that validators and their cooperators are penalized proportionally to their stake when they are
//!   found to be in violation of network rules. This helps maintain fairness in the slashing mechanism
//!   while reinforcing the responsibility of both validators and cooperators.
//! - **Validator Removal for Recent Offences**: If a validator commits an offence during the most recent
//!   slashing span, the `kick_out_if_recent` function is used to remove them from the active set, ensuring
//!   that validators who pose an immediate risk to network security are promptly dealt with.
//! - **Maintaining Network Integrity**: The `add_offending_validator` function helps maintain the integrity
//!   of the network by ensuring that validators who repeatedly violate the protocol are appropriately
//!   disabled, preventing them from continuing to act as active participants in the network.
//!
//! # Integration Considerations
//!
//! - **Flexible Disabling Policies**: The integration of the `DisablingStrategy` allows governance to
//!   determine how strict the policies should be for disabling validators. This flexibility ensures that
//!   networks can adapt their slashing policies based on evolving requirements or changes in governance
//!   sentiment regarding validator behavior.
//! - **Span-Based Slashing Records**: Developers need to ensure that slashing spans are managed correctly
//!   in order to maintain accurate records of validator behavior. The span system helps track when
//!   validators are slashed and ensures that recurrent offenders face increasing penalties. Proper
//!   integration with the staking and governance pallets is crucial for accurate record-keeping.
//! - **Testing and Validation**: Since slashing involves financial penalties, rigorous testing is
//!   necessary to ensure that all edge cases are handled appropriately. Functions like `fetch_spans`
//!   and `slash_cooperators` should be tested in a variety of scenarios, including validators with
//!   complex slashing histories, to ensure that penalties are applied fairly and consistently.
//!
//! # Example Scenario
//!
//! Suppose a validator is detected engaging in behavior that violates network rules, such as double-signing
//! or being offline for extended periods. The `slash_stake` function is invoked to apply a penalty to their
//! stake, reducing their active and unlocking balances proportionally. If the validator has cooperators who
//! have bonded their tokens with the misbehaving validator, the `slash_cooperators` function is also called
//! to apply a penalty to the cooperators' stake. If this misbehavior occurs during the most recent slashing
//! span, the validator is removed from the active set using `kick_out_if_recent`. This series of actions
//! ensures that both the validator and any cooperators are held accountable for the offence, maintaining the
