...
//! # Module Overview
//!
//! This module contains unit tests for the staking pallet of a Substrate-based blockchain. It includes
//! various test cases designed to validate the correct behavior of slashing, validator disabling, reward
//! payouts, and other key staking functionalities. The tests are structured to ensure that critical
//! operations, such as handling validators, cooperators, and slashing spans, are performed accurately
//! and align with the blockchain's economic and governance model.
//!
//! # Key Features and Functions
//!
//! - **Ledger Slashing Tests**:
//!   - The module tests various scenarios where validators are slashed, ensuring that both active and
//!     unlocking funds are slashed proportionally based on the defined rules. The tests verify that
//!     ledger data, such as `total` and `unlocking` amounts, are correctly updated after a slash.
//!
//! - **Validator Disabling Strategy Tests**:
//!   - `dont_disable_for_ancient_offence()`: Tests that a validator is not disabled for offences
//!     committed in a previous, non-current slashing era. This ensures that validators are only penalized
//!     for recent and relevant misbehavior.
//!   - `dont_disable_beyond_byzantine_threshold()`: Ensures that validators are not disabled beyond
//!     the byzantine fault tolerance threshold, maintaining network stability by avoiding excessive
//!     validator removal.
//!   - `disable_when_below_byzantine_threshold()`: Verifies that a validator is disabled when the
//!     number of previously disabled validators is below the byzantine threshold, demonstrating that
//!     the disabling strategy is correctly applied.
//!
//! - **Cooperator and Slash Management**:
//!   - Tests cover scenarios where cooperators of slashed validators are also penalized. This ensures
//!     that cooperators are held accountable for their associations with misbehaving validators, reinforcing
//!     the responsibility of cooperators to select their validators carefully.
//!
//! # Access Control and Security
//!
//! - **Testing Environment**: All the tests are executed within a mock runtime environment (`TestExternalities`).
//!   This ensures that no real funds are at risk and that the blockchain's state can be easily reset or
//!   manipulated between tests.
//! - **Byzantine Threshold Control**: The tests involving disabling strategies ensure that validator
//!   removals respect the byzantine threshold, a critical parameter for maintaining consensus stability
//!   and preventing malicious or accidental majority control of the network.
//!
//! # Developer Notes
//!
//! - **Mock Setup for Consistent State**: Each test uses `sp_io::TestExternalities` to create a consistent
//!   and isolated runtime state. This guarantees that the tests do not interfere with each other, providing
//!   reliable and reproducible outcomes.
//! - **Use of BTreeMap for Slashing Span Records**: The tests use `BTreeMap` to track slashing spans and
//!   ensure that all slashes are recorded accurately. This helps validate that validators with repeated
//!   offences are appropriately penalized, and that historical slashing records are preserved.
//! - **Validation of Recent Offences Only**: The tests are structured to ensure that only recent validator
//!   misbehaviors lead to disabling or further penalties. This prevents validators from being unfairly
//!   punished for offences that occurred far in the past, supporting fair governance practices.
//!
//! # Usage Scenarios
//!
//! - **Slashing Logic Verification**: The tests involving `slash_stake` and `slash_cooperators` are designed
//!   to verify that slashing operations are applied consistently, ensuring that all affected balances (active,
//!   unlocking, and total) are updated correctly. This is crucial for maintaining the integrity of the slashing
//!   mechanism and ensuring that penalties reflect the network's slashing policy.
//! - **Validator Disabling Scenarios**: By testing various disabling scenarios, such as with `dont_disable_beyond_byzantine_threshold()`,
//!   developers can verify that the pallet's logic for handling misbehaving validators aligns with network
//!   requirements. This helps maintain proper participation without destabilizing the validator set due to
//!   excessive disabling actions.
//! - **Era-Specific Behavior**: Tests such as `dont_disable_for_ancient_offence()` help ensure that the
//!   staking pallet only enforces penalties relevant to the current era. This is particularly important for
//!   ensuring that validators are not penalized retroactively, which could undermine confidence in the fairness
//!   of the network's governance mechanisms.
//!
//! # Integration Considerations
//!
//! - **Slashing Span Management**: The tests validate that slashing spans are correctly managed over time,
//!   ensuring that validators with repeated misbehaviors face increasing penalties. Integration with the
//!   slashing and staking pallets must preserve this behavior to maintain an effective penalty system.
//! - **Disabling Strategy Flexibility**: The use of a `DisablingStrategy` provides flexibility in how the
//!   network handles validator misbehavior. The integration should ensure that governance bodies can adjust
//!   the strategy as needed to respond to changing network conditions or governance policies without breaking
//!   the expected behavior tested here.
//! - **Performance Considerations**: Some of the tests involve creating large sets of validators or managing
//!   multiple slashing spans. Developers integrating this code should consider the performance impact of such
//!   operations in a live network setting, especially during slashing or disabling events that may involve
//!   numerous actors.
//!
//! # Example Scenario
//!
//! Suppose a validator is found to have committed an offence during the current era, and governance rules
//! dictate that they should be removed from the active set if their actions endanger network security.
//! The test `disable_when_below_byzantine_threshold()` verifies that this validator will be disabled if the
//! number of currently disabled validators is below the byzantine threshold. Conversely, the test
//! `dont_disable_beyond_byzantine_threshold()` ensures that no further disabling occurs if the threshold
//! has already been met, helping to prevent excessive validator loss and maintain network stability.
//!
...
fn rewards_should_work() {
    ExtBuilder::default().session_per_era(3).build_and_execute(|| {
        ...
        Payee::<Test>::insert(31, RewardDestination::Controller);
        ...
        ReputationPallet::force_set_points(
            RuntimeOrigin::root(),
            31,
            ReputationTier::Trailblazer(1).into(),
        )
        .unwrap();
        ...
        // Not elected validators must get rewards
        assert_eq_error_rate!(
            Assets::balance(VNRG::get(), 30),
            part_for_30.saturating_mul_int(total_payout_0),
            2
        );
        assert_eq_error_rate!(Balances::total_balance(&31), init_balance_31, 2);
        ...
        assert_eq_error_rate!(
            Assets::balance(VNRG::get(), 30),
            part_for_30.saturating_mul_int(total_payout_0 + total_payout_1),
            2
        );
        assert_eq_error_rate!(Balances::total_balance(&31), init_balance_31, 2);
        assert_eq_error_rate!(
...
fn staking_should_work() {
    ExtBuilder::default().no_cooperate().build_and_execute(|| {
        // remember + compare this along with the test.
        assert_eq_uvec!(validator_controllers(), [20, 10]);
        ...
        assert_eq_uvec!(validator_controllers(), [4, 20]);
...
fn cooperators_also_get_slashed_pro_rata() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert!(PowerPlant::ledger(100).unwrap().active < cooperator_stake);
            assert!(PowerPlant::ledger(10).unwrap().active < validator_stake);

            let exposed_stake = initial_exposure.total;
            let slash_amount = slash_percent * exposed_stake;
            let validator_stake_slash =
                Perbill::from_rational(validator_stake, exposed_stake) * slash_amount;
            let cooperator_stake_slash =
                Perbill::from_rational(cooperator_stake, exposed_stake) * slash_amount;

            // both slash amounts need to be positive for the test to make sense.
            assert!(validator_stake_slash > 0);
            assert!(cooperator_stake_slash > 0);

            // as well the balance
            assert_eq!(balances(&101).0, cooperator_balance - cooperator_stake_slash);
            assert_eq!(balances(&11).0, validator_balance - validator_stake_slash);
...
fn switching_roles() {
    ...
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            // with current cooperators 10 and 6 have the most stake
            assert_eq_uvec!(validator_controllers(), [6, 10]);
            ...
            assert_eq_uvec!(validator_controllers(), [6, 2]);
        });
}
...
fn reward_validator_slashing_validator_does_not_overflow() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        let slash = SlashEntityOf::<Test>::max_slash_amount(&reputation.into(), 0).reputation;
...
fn reward_from_authorship_event_handler_works() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_eq_uvec!(Session::validators(), [11, 21]);
...
fn slashing_performed_according_exposure() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        let slash = SlashEntityOf::<Test>::max_slash_amount(&init_reputation_11.into(), 0)
            .reputation
            .deref()
            / 2;
...
fn subsequent_reports_in_same_span_pay_out_less() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        let stake_reward = (initial_balance / 5) / 20;
        assert_eq!(Balances::free_balance(1), 10 + stake_reward);
        ...
        let stake_prior_payout = stake_reward;

        // F1 * (reward_proportion * slash - prior_payout)
        // 50% * (10% * (initial_balance / 2) - prior_payout)
        let stake_reward = ((initial_balance / 20) - stake_prior_payout) / 2;
        assert_eq!(Balances::free_balance(1), 10 + stake_prior_payout + stake_reward);
    });
}
...
fn invulnerables_are_not_slashed() {
    ...
    ExtBuilder::default().invulnerables(vec![11]).build_and_execute(|| {
        ...
        let initial_balance_21 = PowerPlant::slashable_balance_of(&21);
        ...
        let (cooperator_reputations, cooperator_balances): (Vec<_>, Vec<_>) = exposure_21
            .others
            .iter()
            .map(|o| {
                (
                    *ReputationPallet::reputation(o.who).unwrap().reputation.points(),
                    Balances::free_balance(o.who),
                )
            })
            .unzip();
        ...
        let slash_21 = SlashEntity::max_slash_amount(&initial_reputation_21.into(), 0)
            .reputation
            .deref()
            * 2
            / 10;
        ...
        assert_eq!(Balances::free_balance(21), 2000 - (2 * initial_balance_21 / 10));
        ...
        for (initial_reputation, initial_balance, other) in cooperator_reputations
            .into_iter()
            .zip(cooperator_balances)
            .zip(exposure_21.others)
            .map(|((a, b), c)| (a, b, c))
        {
            assert_eq!(
...
fn garbage_collection_after_slashing() {
    ...
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            let slash_11 =
                SlashEntityOf::<Test>::max_slash_amount(&initial_reputation_11.into(), 0)
                    .reputation
                    .deref()
                    / 10;
            ...
            assert_eq_error_rate!(
                *SpanSlash::<Test>::get((11, 0)).amount().reputation,
                slash_11,
                1
            );
            assert_eq!(SpanSlash::<Test>::get((11, 0)).amount().stake, 200);
            ...
            let slash_11 = *(SlashEntity::max_slash_amount(&reputation_11, 0).reputation);
            ...
            assert_eq!(SpanSlash::<Test>::get((11, 0)).amount(), &SlashEntityOf::<Test>::zero());
        })
}
...
fn garbage_collection_on_window_pruning() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        let exposure = PowerPlant::eras_stakers(now, 11);
        ...
        assert_eq!(Balances::free_balance(101), 2000 - (cooperated_value / 10));
...
fn slashes_are_summed_across_spans() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_eq!(Balances::free_balance(21), 2000);
        assert_eq!(PowerPlant::slashable_balance_of(&21), 1000);
        ...
        let reputation_slash_21 =
            SlashEntityOf::<Test>::max_slash_amount(&initial_reputation_21.into(), 0)
                .reputation
                .deref()
                / 10;
        assert_eq!(get_span(21).iter().collect::<Vec<_>>(), expected_spans);
        assert_eq_error_rate!(
            ...
            initial_reputation_21 - reputation_slash_21,
            2
        );
        ...
        assert_eq!(PowerPlant::slashable_balance_of(&21), 900);
        ...
        let reputation_slash_21 =
            SlashEntityOf::<Test>::max_slash_amount(&before_slash_21.into(), 0)
                .reputation
                .deref()
                / 10;
        assert_eq!(get_span(21).iter().collect::<Vec<_>>(), expected_spans);
        assert_eq_error_rate!(
            *ReputationPallet::reputation(21).unwrap().reputation.points(),
            reputation_after_slash - reputation_slash_21 + reputation_per_era(),
            2
        );
...
fn retroactive_deferred_slashes_one_before() {
    ExtBuilder::default().slash_defer_duration(2).build_and_execute(|| {
        ...
        assert_eq!(PowerPlant::ledger(10).unwrap().total, 1000);
        assert_eq!(
            *ReputationPallet::reputation(11).unwrap().reputation.points(),
        ...
        assert_eq!(PowerPlant::ledger(10).unwrap().total, 900);
        assert_ok!(PowerPlant::unbond(RuntimeOrigin::signed(10), 1000));
        assert_eq!(PowerPlant::ledger(10).unwrap().total, 900);
    })
}
...
fn remove_deferred() {
    ExtBuilder::default().slash_defer_duration(2).build_and_execute(|| {
        ...
        let initial_slash = slash_10 * cooperated_value;

        let total_slash = slash_15 * cooperated_value;
...
fn claim_reward_at_the_last_era_and_no_double_claim_and_invalid_claim() {
    ...
    ExtBuilder::default().default_cooperate().build_and_execute(|| {
