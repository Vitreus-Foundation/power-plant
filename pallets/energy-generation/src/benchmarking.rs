...
//! # Module Overview
//!
//! This Rust module defines a set of benchmarking functions for the staking pallet in a
//! Substrate-based blockchain. These benchmarking scenarios are used to evaluate the performance
//! of critical staking operations under realistic conditions, such as slashing, payout of stakers,
//! and handling slashing spans. These benchmarks help ensure that staking operations are efficient,
//! resilient, and capable of handling high loads typical of a live blockchain network.
//!
//! # Key Features and Functions
//!
//! - **Benchmarking Staking Operations**:
//!   - `payout_stakers_dead_controller`: Benchmarks the payout process for validators with cooperators
//!     whose controller accounts have been terminated (dead). This function validates the proper
//!     transfer of staking rewards even if some cooperating accounts are no longer active. It
//!     verifies that payout occurs accurately to the validator controller account, and any inactive
//!     cooperators do not improperly receive rewards.
//!   - `payout_stakers_alive_staked`: Evaluates the staking payout process where the cooperators
//!     are alive and fully staked. This scenario is crucial for measuring the efficiency and correctness
//!     of payouts in the most common case, ensuring that rewards are correctly distributed to both the
//!     validator and cooperators based on their stake and preferences.
//!
//! - **Deferred Slash Management**:
//!   - `cancel_deferred_slash`: Benchmarks the process of canceling deferred slashes for a given era,
//!     which requires removing specific slashes from the `UnappliedSlashes` storage. The benchmark
//!     measures the performance impact of managing and modifying slashing data, which is important for
//!     ensuring a responsive and fair governance mechanism.
//!
//! - **Slashing Span and Validator Management**:
//!   - `add_slashing_spans_works`: Tests the addition of multiple slashing spans for a validator, and
//!     subsequently verifies that these spans can be correctly cleaned up. This benchmarking scenario
//!     measures the system’s ability to manage multiple slashing records efficiently, a common occurrence
//!     for misbehaving validators.
//!
//! - **Benchmark Setup and Execution**:
//!   - `test_payout_all()`: Tests the execution of the `payout_all` benchmark, which simulates distributing
//!     rewards across multiple validators and cooperators. It utilizes the benchmarking framework to assess
//!     the correctness and performance of the payout logic across a substantial number of stakers.
//!
//! # Access Control and Security
//!
//! - **Benchmarking Privileges**: These functions are only used during benchmarking, which means they
//!   are not accessible during normal runtime operations. This distinction is important to prevent
//!   potential misuse of benchmarking scenarios in production environments. Only trusted administrators
//!   should have access to trigger these benchmarks.
//!
//! - **Validation in Benchmarks**: Each benchmarking function includes multiple checks to ensure that
//!   the operations are executed correctly, such as validating that balances have increased after payouts,
//!   or that slashing spans have been correctly added and removed. This ensures that the benchmarks
//!   not only measure performance but also verify correctness.
//!
//! # Developer Notes
//!
//! - **Whitelisted Callers**: The benchmarking setup often uses a `whitelisted_caller()` to simulate a
//!   trusted caller, which helps in creating realistic scenarios while ensuring that only authorized
//!   interactions are benchmarked.
//! - **Edge Cases**: The benchmarking scenarios cover edge cases, such as dead controller accounts or
//!   adding numerous slashing spans. These edge cases are important to assess the system’s behavior under
//!   less common but possible circumstances, thereby improving the overall robustness of the staking system.
//! - **Extensive Logging and Validation**: Benchmarks include checks and validations to assert the
//!   correctness of the changes made during staking operations, such as verifying payouts or ensuring
//!   the cleanup of slashing spans. These validations are crucial for ensuring that the benchmarks provide
//!   reliable insights into both performance and correctness.
//!
//! # Usage Scenarios
//!
//! - **Validator Payouts with Dead Cooperators**: The `payout_stakers_dead_controller` benchmark can be
//!   used to test the payout system when certain cooperators are no longer active. This ensures that
//!   the system can handle payouts without errors or delays, even when dealing with inactive accounts.
//! - **Handling Slash Cancellations**: The `cancel_deferred_slash` function is used to benchmark the
//!   system’s capability to cancel scheduled slashes. This is crucial during governance decisions where
//!   a slash might be determined to be unjust or incorrect and needs to be removed quickly and safely.
//! - **Managing Slashing Spans**: The `add_slashing_spans_works` benchmark is used to ensure that the
//!   system can efficiently add and manage slashing spans for validators. This scenario is particularly
//!   useful for validators with repeated offences and helps in understanding the performance impact of
//!   managing a large number of slashing records.
//!
//! # Integration Considerations
//!
//! - **Realistic Parameters**: The benchmarks use a variety of parameters, such as different numbers of
//!   cooperators or varying slashing spans, to simulate realistic network conditions. Developers should
//!   consider these parameters to represent expected mainnet conditions to get accurate performance
//!   metrics.
//! - **Payout Logic Verification**: The `payout_stakers` benchmarks are designed to thoroughly test the
//!   reward distribution logic, which is one of the core components of the staking pallet. Ensuring this
//!   logic is both efficient and correct is critical for maintaining staker trust and incentivizing
//!   network participation.
//! - **Benchmark Configuration**: To use these benchmarks effectively, developers should compile with
//!   the `runtime-benchmarks` feature enabled. This ensures that the benchmarking functions are included
//!   in the build and can be executed to gather performance metrics.
//!
//! # Example Scenario
//!
//! Suppose a network upgrade requires testing how efficiently the staking pallet can handle payouts to
//! validators and cooperators across different conditions, such as dead controllers or a large number of
//! stakers. The benchmarking functions in this module—like `payout_stakers_dead_controller` and
//! `payout_stakers_alive_staked`—can be used to simulate these scenarios and collect data on execution
//! times and potential bottlenecks. Additionally, in cases where validators are slashed, the benchmarks
//! for adding and removing slashing spans (`add_slashing_spans_works`) provide insights into how well
//! the system can manage the associated bookkeeping.
//!
...
type MaxCooperators<T> = <<T as Config>::BenchmarkingConfig as BenchmarkingConfig>::MaxCooperators;
...
// This function clears all existing validators and cooperators from the set, and generates one new
// validator being cooperated by n cooperators, and returns the validator stash account and the
// cooperators' stash and controller. It also starts an era and creates pending payouts.
pub fn create_validator_with_cooperators<T: Config>(
    n: u32,
    upper_bound: u32,
    dead: bool,
    ...
    let original_cooperator_count = Cooperators::<T>::count();
    let mut cooperators = Vec::new();

    // Give the validator n cooperators, but keep total users in the system the same.
    for i in 0..upper_bound {
        ...
        if i < n {
            ...
            cooperators.push((n_stash, n_controller));
        }
    }
    ...
    assert_eq!(Cooperators::<T>::count(), original_cooperator_count + cooperators.len() as u32);
...
benchmarks! {
    ...
    bond_extra {
        ...
        let origin_weight = MinCooperatorBond::<T>::get().max(T::Currency::minimum_balance());

        // setup the worst case list scenario.

        // the weight the cooperator will start at.
    ...
    unbond {
        ...
        // setup the worst case list scenario.
        let total_issuance = T::Currency::total_issuance();
        // the weight the cooperator will start at. The value used here is expected to be
    ...
    kick {
        ...
        let rest_of_validators = create_validators_with_seed::<T>(T::MaxCooperations::get() - 1, 100, 415)?;
        ...
        // we now create the cooperators. there will be `k` of them; each will cooperate all
        // validators. we will then kick each of the `k` cooperators from the main validator.
        let mut cooperator_stashes = Vec::with_capacity(k as usize);
        for i in 0 .. k {
            // create a cooperator stash.
            let (n_stash, n_controller) = create_stash_controller::<T>(
                T::MaxCooperations::get() + i,
                100,
                Default::default(),
            )?;

            // bake the cooperations; we first clone them from the rest of the validators.
            let mut cooperations = rest_of_validators.clone();
            // then insert "our" validator somewhere in there (we vary it) to avoid accidental
            // optimisations/pessimisations.
            cooperations.insert(i as usize % (cooperations.len() + 1), stash_lookup.clone());
            // then we cooperate.
            Staking::<T>::cooperate(RawOrigin::Signed(n_controller.clone()).into(), cooperations)?;

            cooperator_stashes.push(n_stash);
        }

        // all cooperators now should be cooperating our validator...
        for n in cooperator_stashes.iter() {
            assert!(Cooperators::<T>::get(n).unwrap().targets.contains(&stash));
        }

        // we need the unlookuped version of the cooperator stash for the kick.
        let kicks = cooperator_stashes.iter()
    ...
    cooperate {
        ...
        let (stash, controller) = create_stash_controller_with_balance::<T>(
            SEED + T::MaxCooperations::get() + 1, // make sure the account does not conflict with others
            origin_weight,
            Default::default(),
        ).unwrap();
    ...
    payout_stakers_dead_controller {
        let n in 0 .. T::MaxCooperatorRewardedPerValidator::get() as u32;
        let (validator, cooperators) = create_validator_with_cooperators::<T>(
            n,
            T::MaxCooperatorRewardedPerValidator::get() as u32,
            true,
        ...
        for (_, controller) in &cooperators {
    ...
    payout_stakers_alive_staked {
        ...
        let mut cooperator_balances_before = Vec::new();
        for (stash, _) in &cooperators {
            let balance = T::Currency::free_balance(stash);
            cooperator_balances_before.push(balance);
        }
    ...
    verify {
        ...
        for ((stash, _), balance_before) in cooperators.iter().zip(cooperator_balances_before.iter()) {
            let balance_after = T::Currency::free_balance(stash);
            ensure!(
                balance_before < &balance_after,
                "Balance of cooperator stash should have increased after payout.",
            );
        }
    }
    ...
    rebond {
        ...
        let origin_weight = MinCooperatorBond::<T>::get()
    ...
    new_era {
        ...
        create_validators_with_cooperators_for_era::<T>(
            v,
            n,
            <T as Config>::MaxCooperations::get() as usize,
            false,
            None,
        )?;
    ...
    get_npos_voters {
        ...
        // number of cooperator intention. we will iterate all of them.
        let n in (MaxCooperators::<T>::get() / 2) .. MaxCooperators::<T>::get();

        let validators = create_validators_with_cooperators_for_era::<T>(
            v, n, T::MaxCooperations::get() as usize, false, None
        )?
        .into_iter()
        ...
        assert_eq!(Cooperators::<T>::count(), n);
    ...
    get_npos_targets {
        ...
        let v in (MaxValidators::<T>::get() / 2) .. MaxValidators::<T>::get();
        // number of cooperator intention.
        let n = MaxCooperators::<T>::get();

        let _ = create_validators_with_cooperators_for_era::<T>(
            v, n, T::MaxCooperations::get() as usize, false, None
        )?;
    }: {
    ...
    set_staking_configs_all_set {
        ...
        assert_eq!(MinCooperatorBond::<T>::get(), BalanceOf::<T>::max_value());
        assert_eq!(MinValidatorBond::<T>::get(), BalanceOf::<T>::max_value());
        assert_eq!(MaxCooperatorsCount::<T>::get(), Some(u32::MAX));
    ...
    set_staking_configs_all_remove {
        ...
        assert!(!MinCooperatorBond::<T>::exists());
        assert!(!MinValidatorBond::<T>::exists());
        assert!(!MaxCooperatorsCount::<T>::exists());
...
mod tests {
    ...
    #[test]
    fn create_validators_with_cooperators_for_era_works() {
        ExtBuilder::default().build_and_execute(|| {
            let v = 10;
            let n = 100;

            create_validators_with_cooperators_for_era::<Test>(
                v,
                n,
                <Test as Config>::MaxCooperations::get() as usize,
                false,
                None,
            )
            .unwrap();
            ...
            let count_cooperators = Cooperators::<Test>::iter().count();

            assert_eq!(count_validators, Validators::<Test>::count() as usize);
            assert_eq!(count_cooperators, Cooperators::<Test>::count() as usize);

            assert_eq!(count_validators, v as usize);
            assert_eq!(count_cooperators, n as usize);
        });
    }

    #[test]
    fn create_validator_with_cooperators_works() {
        ExtBuilder::default().build_and_execute(|| {
            let n = 10;

            let (validator_stash, cooperators) = create_validator_with_cooperators::<Test>(
                n,
            ...
            assert_eq!(cooperators.len() as u32, n);
    ...
    fn add_slashing_spans_works() {
        ExtBuilder::default().build_and_execute(|| {
            ...
            let (validator_stash, _cooperators) = create_validator_with_cooperators::<Test>(
                n,
                <<Test as Config>::MaxCooperatorRewardedPerValidator as Get<_>>::get(),
                false,
                RewardDestination::Staked,
