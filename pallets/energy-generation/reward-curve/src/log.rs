//! Logarithm Implementation for Reward Curve Generation
//!
//! This module provides a fixed-point implementation of the binary logarithm (log2)
//! function using Taylor series approximation. It is specifically designed for the
//! reward curve generation system, handling rational fractions with high precision.
//!
//! # Implementation Details
//!
//! The logarithm is computed in three steps:
//! 1. Find the nearest power of 2 binary approximation
//! 2. Use Taylor series to compute the residual
//! 3. Combine results for final fixed-point output
//!
//! ## Mathematical Formula
//! ```text
//! For input p/q where p/q ∈ [1, 1_000_000]:
//! log2(p/q) = n + log2((p - 2^n*q)/(p + 2^n*q))
//! where n is chosen so that q * 2^n <= p < q * 2^(n+1)
//! ```
//!
//! # Constraints
//!
//! - Input ratio p/q must be in range [1, 1_000_000]
//! - Both p and q must be ≤ 1_000_000
//! - Results are expressed in millionths (10^-6 precision)
//!
//! # Security Notes
//!
//! - Includes bounds checking for all inputs
//! - Prevents overflow in intermediate calculations
//! - Validates power-of-2 computations
//!
//! # Testing
//!
//! Comprehensive tests verify:
//! - Accuracy against floating-point calculations
//! - Boundary condition handling
//! - Error cases and panics
...
//! ```rust
//! # use self::log2;
//! // Calculate log2(1_000_000/1_000) with fixed-point precision
//! let result = log2(1_000_000, 1_000); // Returns result in millionths
//! ```
//!
//! # Note
//!
//! This implementation optimizes for the specific use case of reward curve
//! generation. It is not intended as a general-purpose logarithm function.
//! Accuracy is guaranteed to within 10^-6 for the supported input range.
