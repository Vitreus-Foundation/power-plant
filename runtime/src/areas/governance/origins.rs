...
pub mod pallet_custom_origins {
	use crate::{Balance, NANO_VTRS, PICO_VTRS, MICRO_VTRS, MILLI_VTRS};
	...
	pub enum Origin {
		...
		StakingAdmin,
		/// Origin for spending up to 10,000,000 VTRS from the treasury as well as generally
		/// administering it.
		Treasurer,
	...
	decl_ensure! {
		pub type Spender: EnsureOrigin<Success = Balance> {
			SmallTipper = 2_500 * NANO_VTRS,
			BigTipper = 10 * MICRO_VTRS,
			SmallSpender = 100 * MICRO_VTRS,
			MediumSpender = 1 * MILLI_VTRS,
			BigSpender = 10 * MILLI_VTRS,
			Treasurer = 100 * MILLI_VTRS,
		}
	}
}
